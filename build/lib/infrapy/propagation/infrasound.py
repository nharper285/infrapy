        ...
        for n_az in range(self.az_bin_cnt):
            ...
            for nr in range(rng_cnt):
                ...
                if az_dev_multi_bounce[rng_mask].shape[0] > min_pts:
                    az_dev_mns[n_az][nr] = np.mean(az_dev_multi_bounce[rng_mask], dtype=float64)
                    az_dev_vrs[n_az][nr] = max(np.sqrt(np.var(az_dev_multi_bounce[rng_mask], dtype=float64)), self.min_az_dev_var)
                else:
                    if nr == 0:
                ...
                if rcel_multi_bounce[tropo_mask].shape[0] > min_pts:
                    rcel_mns[n_az][nr][0] = np.mean(rcel_multi_bounce[tropo_mask], dtype=float64)
                    rcel_vrs[n_az][nr][0] = max(np.sqrt(np.var(rcel_multi_bounce[tropo_mask], dtype=float64)) * 1.5, self.rcel_vrs_min)
                else:
                    if nr == 0:
                ...
                if rcel_multi_bounce[strat_mask].shape[0] > min_pts:
                    rcel_mns[n_az][nr][1] = np.mean(rcel_multi_bounce[strat_mask], dtype=float64)
                    rcel_vrs[n_az][nr][1] = max(np.sqrt(np.var(rcel_multi_bounce[strat_mask], dtype=float64)) * 1.5, self.rcel_vrs_min)
                else:
                    if nr == 0:
                ...
                if rcel_multi_bounce[therm_mask].shape[0] > min_pts:
                    rcel_mns[n_az][nr][2] = np.mean(rcel_multi_bounce[therm_mask], dtype=float64)
                    rcel_vrs[n_az][nr][2] = max(np.sqrt(np.var(rcel_multi_bounce[therm_mask], dtype=float64)) * 1.5, self.rcel_vrs_min)
                else:
                    if nr == 0:
