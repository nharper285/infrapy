...
if __name__ == '__main__':
    ...
    for tr in stream:
        print(tr.stats.network + "." + tr.stats.station + "." + tr.stats.location + "." + tr.stats.channel + '\t' + str(tr.stats.starttime) + " - " + str(tr.stats.endtime))
    ...
    for m in range(M):
        ...
        plt.xlim([0, t[-1]])
        plt.plot(t, x[m], 'k-')
        plt.axvspan(xmin = signal_start , xmax = signal_end, alpha = 0.25, color = 'blue')
        if method == "gls":
    ...
    # det_vis.plot_fk1(read(sac_glob), lat_lon, times, beam_results, output_path="KAUT/beamform_results_v2.png")
    # time.sleep(100)
    ...
    # plt.figure(3).savefig("KAUT/beamform_results_burndown.png")
    # plt.figure(4).savefig("KAUT/beamform_results_final.png")
    # plt.pause(230.0)

    # # plt.savefig("KAUT/beam_results.png")
    # plt.close()
    # p.close()
